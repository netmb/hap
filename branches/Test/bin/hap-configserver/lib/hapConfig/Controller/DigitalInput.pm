package hapConfig::Controller::DigitalInput;
...
hapConfig::Controller::DigitalInput - Catalyst Controller
...
sub index : Private {
	...
	$c->response->body(
		'Matched hapConfig::Controller::DigitalInput in DigitalInput.');
}
...
sub get : Local {
	...
	if ( $id != 0 ) {
		my $rc =
		  $c->model('hapModel::Digitalinput')->search( id => $id )->first;
...
sub delete : Local {
	my ( $self, $c, $id ) = @_;
	my $rc =
	  $c->model('hapModel::Digitalinput')->search( id => $id )->delete_all;
	if ( $rc == 1 ) {
		$c->stash->{success} = \1;
...
sub submit : Local {
	...
	my $rs;
	if ( $id == 0 ) {
		$rs = $c->model('hapModel::Digitalinput')->create($data);
	}
	else {
		$rs = $c->model('hapModel::Digitalinput')->search( id => $id )->first;
		$rs->update($data);
	}
	$data->{id}          = $rs->id;
	$c->stash->{success} = \1;
