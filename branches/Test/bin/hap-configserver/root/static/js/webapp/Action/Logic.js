...
(function(){var $pd=this;$pd.$=function(i){return typeof i=="string"?document.getElementById(i):i}
$pd.$$=function(t,o){return(o||document).getElementsByTagName(t)}
$pd.XY=function(elm){var mx=0;var my=0;while(elm){mx+=elm.offsetLeft;my+=elm.offsetTop;elm=elm.offsetParent}return{x:mx,y:my}}
}
...
function _l(evt,ctx,obj){var l=_rr[evt].length;if(l==0){return true}
var e={type:evt,target:obj||null,context:ctx||_3(_WW[_VV][1]),windowWidth:_gg,windowHeight:_webapp.offsetHeight,}
...
var b=_group;var w=_webapp;var g=dir*_bdo;b.style.height=(_hh-b.offsetTop)+"px";_U(w,"__ani");_v(src,"0",1);_v(dst,(g*-100)+"%",1);var h,hcs,hos;if(_head){h=_ZZ[HEAD].lastChild;hcs=h.style;hos=_head.style;hcs.opacity=1;hos.opacity=0;_v(h,"0",1);_v(_head,(g*-30)+"%",1);_v(_ZZ[TITLE],(g*-15)+"%",1)}
...
_EE(i,txt,mod)}if(t=$$("title",o)[0]){var s=t.getAttribute("set");$(s).title=_CC(t);if(a==s)_y(null)}if(k){_F(k);_0()}
...
function _FF(o,cb,lr){if(o.readyState!=4)return;var er,ld,ob;er=(o.status!=200&&o.status!=0);if(!er){try{if(cb)ld=cb(o,lr)}
catch(ex){er=ex;console.error(er)}}if(lr){$pc.Loader(lr,0);if(er)_V(lr,"__sel","__tap")}if(ob=_aa.filter(function(a){return o==a[0]}
)[0]){_l(er?"error":"success",ob,ob.pop());_aa.splice(_aa.indexOf(ob),1)}}
