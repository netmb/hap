...
int DMCalcZZPPAb(double px) {
  return -ceil(DMTicsProHalbwelle / M_PI * acos(2 * px / DMSteps - 1));
}
...
void DMSetValue(tByte pX, tByte pPHW, tWord pDelay) {
  ...
  if(Delay == 0) {
    ...
    if(DMS.E[pX].Prop & KMIODMPAb)
      DMSetZZP(DMTicsSynchDiff - DMTicsProHalbwelle + 10, DMCalcZZPPAb(HW), pX);
    else {
      ZZP = DMCalcZZPPAn(HW);
...
tByte DMGetValue(tByte pX) {
  return DMS.E[pX].HW * 100 / DMSteps + .5;
}
      ...
      if(DMS.E[i].DC == 0) {
        if(DMS.E[i].HW < DMS.E[i].HWNew) {
          ...
          if(DMS.E[i].Prop & KMIODMPAb)
            DMSetZZP(DMTicsSynchDiff - DMTicsProHalbwelle + 10, DMCalcZZPPAb(DMS.E[i].HW), i);
          else {
